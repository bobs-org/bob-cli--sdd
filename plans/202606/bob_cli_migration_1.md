---
create_time: 2026-06-01 09:52:26
status: wip
prompt: .sase/sdd/plans/202606/prompts/bob_cli_migration_1.md
tier: epic
---
...
Adjacent Bob integration code also exists in Neovim config/tests:
...
- `tests/bash/bob_pomodoro_runtimes_test.sh`
...
- Preserve existing environment overrides such as `BOB_DIR`, `BOB_DAY_FILE`, `BOB_NOW`, `DATE`, `OB_COMMAND`, and
  `BOB_SYNC_LOCK_FILE`.
- Import the existing `bob_pomodoro_runtimes` Bash test into this repo or stage fixtures that a later phase can use.
...
- A temporary `HOME` without `~/lib/bugyi.sh` can run `scripts/bob_pomodoro --help` or an equivalent no-op/syntax path
  without failing on the missing personal shell library.
...
- Port `tests/bash/bob_pomodoro_runtimes_test.sh` so it runs against the repo-local script or the
  `bob pomodoro-runtimes` command.
